...
const getAllOrders = async ({ status }) => {
  ...
  `;
  const params = [];
  if (status) { query += ` AND o.status = $1`; params.push(status); }
...
const tableCheck = await pool.query('SELECT * FROM tables WHERE id = $1', [tableId]);
  ...
  const ticketCheck = await pool.query(
    'SELECT * FROM buffet_tickets WHERE id = $1 AND is_active = true',
    [buffetTicketId]
  );
 ...
 const result = await pool.query(
  ...
  await pool.query(`UPDATE tables SET status = 'ordering' WHERE id = $1`, [tableId]);
