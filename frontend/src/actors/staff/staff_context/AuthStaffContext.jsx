...
const AuthStaffContext = createContext(null);
...
export const AuthStaffProvider = ({ children }) => {
  ...
  const initSocket = () => {
    ...
    socket.on('connect', () => {
      ...
      socket.emit('join_staff'); // vào room staff
    });

    // Realtime — bàn thay đổi status
    ...
    // Realtime — món thay đổi (để tính pendingTables)
    socket.on('item_status_updated', () => {
      fetchTables(); // fetch lại để cập nhật số món
    });
...
const getTableColor = (table) => {
  ...
  if (table?.status === 'maintenance') return "#B4463C";
  return "#a4a6a5"; // empty
};

const getChairColor = (table) => {
  if (table?.status === 'ordering')    return "#60A5FA";
  if (table?.status === 'maintenance') return "#FCA5A5";
  return "#a1a1a1"; // empty
};
...
export const useAuthStaff = () => useContext(AuthStaffContext);